...
public:
	...
	auto loadMul(const std::filesystem::path &uodir, bool isHS, const tileinfo *tileInfo) ->bool;
	auto size() const ->size_t ;
	auto multis() const -> const std::unordered_map<int, collection_item>& ;
	auto multis()  ->  std::unordered_map<int, collection_item>& ;
	auto operator[](int index) const ->const collection_item& ;
	auto operator[](int index) ->collection_item& ;
	auto exists(int index) const ->bool ;
