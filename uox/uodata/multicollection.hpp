...
public:
	multicollection(const std::filesystem::path &uodir = std::filesystem::path(),tiledata *info = nullptr);
	
	auto load(const std::filesystem::path &uodir,tiledata *info=nullptr) ->bool ;
	auto load(const std::filesystem::path &uodir,const std::string &housingbin,tiledata *info = nullptr) ->bool ;
	auto loadMul(const std::filesystem::path &uodir,tiledata *info = nullptr) ->bool;
