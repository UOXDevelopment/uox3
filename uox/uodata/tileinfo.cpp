...
//=========================================================
tileinfo::tileinfo(const std::filesystem::path &infopath ):_isHS(false){
	if (!infopath.empty()){
		load(infopath);
	}
}
//=========================================================
auto tileinfo::load(const std::filesystem::path &infopath) ->bool {
	auto rvalue = false ;
	_art.clear();
	_terrain.clear();
	
	if (std::filesystem::exists(infopath)){
		_isHS = (std::filesystem::file_size(infopath)>=3188736?true:false);
...
auto tileinfo::processArt(std::ifstream &input) ->void {
	...
	if ( !(input.eof() || input.bad())){
		...
		do {
			...
			info.weight = static_cast<weight_t>(value8);
			input.read(reinterpret_cast<char*>(&value8),1);
			info.layer = static_cast<layer_t>(value8);
			//misc data
			...
			// Animation
			input.read(reinterpret_cast<char*>(&value16),2);
			info.animid = static_cast<animid_t>(value16) ;
			...
			// Height
			input.read(reinterpret_cast<char*>(&value8),1);
			info.height = static_cast<height_t>(static_cast<signed char>(value8)) ;
			// and name
			input.read(string_buffer.data(),20);
			if (input.gcount()==20){
				// We need to find the "0", if any
