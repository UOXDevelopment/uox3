...
//===================================================================
auto socket_t::ipPeer(std::uint32_t ip,bool bigendian) ->std::string {
	auto ptr = reinterpret_cast<unsigned char*>(&ip) ;
	if (bigendian) {
		std::reverse(ptr,ptr+4);
	}
	return std::to_string(ptr[3])+"."s+std::to_string(ptr[2])+"."s+std::to_string(ptr[1])+"."s+std::to_string(ptr[0]);
}
//===================================================================
auto socket_t::startNetwork()->void {
...
//=========================================================
//===================================================================
auto socket_t::stopNetwork()->void {
#if defined(_WIN32)
	WSACleanup();
#endif
}
...
//===================================================================
auto socket_t::peer() const ->std::pair<std::uint32_t,std::uint16_t> {
	...
	auto port = static_cast<std::uint16_t>(peer.sin_port) ;
	auto ip = static_cast<std::uint32_t>(peer.sin_addr.s_addr);
	// Now both are in big endian, so reverse them
	std::reverse(reinterpret_cast<std::uint8_t*>(&port),reinterpret_cast<std::uint8_t*>(&port)+2);
	std::reverse(reinterpret_cast<std::uint8_t*>(&ip),reinterpret_cast<std::uint8_t*>(&ip)+4);
