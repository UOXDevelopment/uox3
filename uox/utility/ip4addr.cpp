...
//=================================================================================
const std::vector<ip4addr_t> ip4addr_t::lanips{
	ip4addr_t("10.*.*.*"s),ip4addr_t("192.168.*.*"s),ip4addr_t("172.16.*.*"s),ip4addr_t("172.17.*.*"s),
	ip4addr_t("172.18.*.*"s),ip4addr_t("172.19.*.*"s),ip4addr_t("172.20.*.*"s),ip4addr_t("172.21.*.*"s),
	ip4addr_t("172.22.*.*"s),ip4addr_t("172.23.*.*"s),ip4addr_t("172.241.*.*"s),ip4addr_t("172.25.*.*"s),
	ip4addr_t("172.26.*.*"s),ip4addr_t("172.27.*.*"s),ip4addr_t("172.28.*.*"s),ip4addr_t("172.29.*.*"s),
	ip4addr_t("172.30.*.*"s),ip4addr_t("172.31.*.*"s)
} ;
//=================================================================================
const std::vector<ip4addr_t> ip4addr_t::localips{
	ip4addr_t("127.*.*.*")
};
//=================================================================================
const std::vector<ip4addr_t> ip4addr_t::apipaips{
	ip4addr_t("169.254.*.*")
};

auto ip4addr_t::exact(const ip4addr_t& value) const ->bool {
	auto rvalue = true ;
	for (auto i = 0; i<4;++i) {
...
//==============================================================
auto ip4addr_t::match(std::uint32_t value, bool bigendian) const ->int {
...
//==============================================================
auto ip4addr_t::match(const ip4addr_t &value) const ->int {
	auto rvalue = 0 ;
	for (auto i = 0 ; i<4 ; ++i){
		rvalue = i+1 ;
...
//=========================================================
ip4addr_t::ip4addr_t(const std::string &value ){
	components.fill("*"s);
...
//=========================================================
auto ip4addr_t::ipaddr(bool bigendian) const ->std::uint32_t {
...
//=========================================================
auto ip4addr_t::type() const ->ip4type_t {
	auto rvalue = ip4type_t::wan ;
	try {
		auto iter = std::find_if(lanips.begin(),lanips.end(),[this](const ip4addr_t& ip){
			return *this == ip ;
		});
		if (iter!= lanips.end()){
			...
			auto iter = std::find_if(localips.begin(),localips.end(),[this](const ip4addr_t& ip){
				return *this == ip ;
			});
			if (iter !=localips.end()){
				...
				auto iter = std::find_if(apipaips.begin(),apipaips.end(),[this](const ip4addr_t& ip){
					return *this == ip ;
				});
...
//=========================================================
auto ip4addr_t::description() const ->std::string {
	auto ip = std::string() ;
...
//=================================================================================
auto ip4list_t::bestmatch(const ip4addr_t &value) const ->std::pair<ip4addr_t,int> {
	auto matches = std::vector<std::pair<ip4addr_t,int>>() ;
	for (const auto &addr: ipaddresses){
		auto comp = addr.match(value);
		matches.push_back(std::make_pair(addr, comp));
	}
	std::sort(matches.begin(),matches.end(),[](const std::pair<ip4addr_t,int> &lhs, const std::pair<ip4addr_t,int> &rhs){
		return lhs.second < rhs.second ;
	});
	return *matches.rbegin();
}
//=================================================================================
auto ip4list_t::bestmatch(std::uint32_t value, bool bigendian ) const ->std::pair<ip4addr_t,int> {
	auto ptr = reinterpret_cast<std::uint8_t*>(&value);
	if (!bigendian){
		std::reverse(ptr, ptr+4);
	}
...
//=================================================================================
auto ip4list_t::add(const ip4addr_t &value) ->void {
	ipaddresses.push_back(value);
}
//=================================================================================
auto ip4list_t::remove(const ip4addr_t &value) ->void {
	auto iter = std::find_if(ipaddresses.begin(),ipaddresses.end(),[value](const ip4addr_t &entry){
						...
						case static_cast<int>(state_t::data):{
							if (line[0] != '}') {
								...
								if ((key=="ip") || (key=="IP") || (key=="Ip") || (key=="iP")) {
									ipaddresses.push_back(ip4addr_t(value));
								}
							}
							else {
...
//=================================================================================
auto ip4list_t::ips() const ->const std::vector<ip4addr_t>& {
	return ipaddresses;
}
//=================================================================================
auto ip4list_t::ips() -> std::vector<ip4addr_t>&{
	return ipaddresses;
}
			...
			if (pUnicast != nullptr) {
				if (pUnicast->Address.lpSockaddr->sa_family == AF_INET) {
					for (i = 0; pUnicast != nullptr; i++) {
						...
						device_address = ip4addr_t(sa_in->sin_addr.S_un.S_addr );
						
						if (device_address.type() != ip4addr_t::ip4type_t::apipa) {
